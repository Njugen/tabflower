...
class CalendarModule extends Module {
  ...
  getDatesOfMonth = (year, month) => {
    let startOfSelectedMonth = Date.parse(
      new Date(this.getMonthAsText(month) + " 01, " + year + " 00:00:00")
    ); // Start of selected Month in milliseconds
  ...
  renderMonthTable = (year, month) => {
      ...
      while (dateCounter < 40) {
        ...
        if (typeof dates[year][month][dateAsKey] === "undefined") {
          ...
          if (dates[year][month][dateAsKey].weekday) {
            for (
              var paddingRunner = 1;
              paddingRunner <= padding;
              paddingRunner++
            ) {
              storeDateJSX(
                <td key={year + "-" + month + "-padding-" + paddingRunner}></td>
              );
            }
            ...
            storeDateJSX(
              <td
                onClick={() => {
                  this.sendToModal({ id: "date-settings", params: {} });
                }}
                className={
                  dateAsKey === currentDate &&
                  month === currentMonth &&
                  year === currentYear
                    ? "activeDate"
                    : ""
                }
                key={year + "-" + month + "-" + dateAsKey}
              >
                {dateCounter}
              </td>
            );
    ...
    return (
      <Fragment>
        <thead>{this.renderMonthTableHeader()}</thead>
        <tbody>{rowsJSX}</tbody>
      </Fragment>
    );
  };
  ...
  browseMonth = (newMonth, newYear) => {
    const {
      month: currentMonth,
      year: currentYear,
    } = this.state.moduleData.selectedDate;
    ...
    return (
      <div className="tabeon-calendar-navigation-section">
        <a
          ...
          className="tabeon-calendar-nav-arrow tabeon-calendar-left-nav fas fa-chevron-left"
          onClick={() => this.browseMonth("previous")}
        ></a>
        <h2 className="tabeon-calendar-title">
          {this.getMonthAsText(month) + " " + year}
        </h2>
        <a
          href={"#" + this.props.urlPath}
          className="tabeon-calendar-nav-arrow tabeon-calendar-right-nav fas fa-chevron-right"
          onClick={() => this.browseMonth("next")}
        ></a>
      </div>
    );
  };
        ...
        <button
          ...
          onClick={() => {
            this.sendToModal({
              id: "confirm-action",
              params: {},
              action: this.saveModalData.bind(this),
            });
          }}
        >
          Save
